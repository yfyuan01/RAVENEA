...
from helper import load_cvqa_data, load_documents, load_retrieval_run
...
def prepare_cvqa_input(args, data, docs_map, retrieval_map):
    QUERY_TEMPLATE = """Answer the following multiple choice question for the image. The last line must be of the following format: 'Answer: $LETTER' (without quotes) where LETTER must be one of A, B, C, or D.{Retrieval}

Question: {Question}

{Options}""".strip()
    
    retrieval_template = """ Use the shared culture between the image and the following document to answer the question.
...
{Retrieval}"""
    ...
    for item in data:
        # Prepare Options
        # item['options'] is something like ["A. Option 1", "B. Option 2", ...]
...
def main(args):
    ...
    data = load_cvqa_data(args.task_path)
    ...
    prompts = prepare_cvqa_input(args, data, docs_map, retrieval_map)
    ...
    sampling_params = SamplingParams(temperature=0, max_tokens=2048, stop_token_ids=req_data.stop_token_ids)
    ...
    # Save predictions
    out_dir = Path("cvqa_outputs")
    out_dir.mkdir(exist_ok=True)
    model_name = args.model_id.split("/")[-1]
    out_file = out_dir / f"results_{model_name}.json"
    if args.use_retrieval:
        retriever_name = args.retrieval_result_path.split("/")[-1].split(".")[0]
        out_file = out_dir / f"results_{model_name}_{args.top_k_retrieval}_{retriever_name}.json"
